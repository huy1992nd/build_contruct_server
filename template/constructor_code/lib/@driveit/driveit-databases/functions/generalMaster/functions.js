...
class Functions {
    ...
    static async getDataByIds(dataArrays) {
        let returnList = [];
        let cacheDataArr = [];
        cacheDataArr.push(dataArrays[0].masterdata);
    ...
    static async getDataByQuery(dataArrays) {
        ...
        _.forEach(dataArrays, (data) => {
            ...
            let foundMasterdata = _.find(mapping, (o, k) => { return o.name === data.masterdata; });
            if (foundMasterdata) {
                let p;
                const model = require(foundMasterdata.modelPath);
